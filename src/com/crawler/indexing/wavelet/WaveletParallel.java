...
public class WaveletParallel implements Wavelet{
	...
	private Node<Byte> createTree(List<Integer> input, int start, int end, byte b) {
		...
		Runnable r1  = new Runnable(){
            public void run(){
                root.left = createTree1(leftList,start,mid,l);
            }
        };
        
        Runnable r2  = new Runnable(){
            public void run(){
                root.right = createTree1(rightList,mid+1,end,r);
            }
        };
	...
	private Node<Byte> createTree1(List<Integer> input, int start, int end, byte b) {
		if(start==end){
            Node<Byte> root = new Node<>();
            for(int i=0;i<input.size();i++)
                root.list.add(b);
        ...
        Runnable r1  = new Runnable(){
            public void run(){
                root.left = createTree2(leftList,start,mid,l);
            }
        };
        
        Runnable r2  = new Runnable(){
            public void run(){
                root.right = createTree2(rightList,mid+1,end,r);
            }
        };
	...
	private Node<Byte> createTree2(List<Integer> input, int start, int end, byte b) {
		if(start==end){
            Node<Byte> root = new Node<>();
            for(int i=0;i<input.size();i++)
                root.list.add(b);
		...
		try (BufferedWriter outputWriter = new BufferedWriter(new FileWriter(outputFilePath));){
			...
			while(!queue.isEmpty()) {
				...
				for (int i = 0; i < size; i++) {
					...
					if (curr.left != null) {
						queue.add(curr.left);
						outputWriter.write(curr.left.list + "\n");
						System.out.println(curr.left.list);
					}
					if (curr.right != null) {
						queue.add(curr.right);
						outputWriter.write(curr.right.list + "\n");
